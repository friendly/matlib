...
This vignette uses an example (from Johnson & Wichern,  Ex. 2.9) of a $3 \times 3$ matrix to illustrate some properties of
...
```{r}
A <- matrix(c(13, -4, 2, -4, 13, -2, 2, -2, 10), 3, 3, byrow=TRUE)
A
```
...
(vectors <- ev$vectors)
```
Note that the eigenvalues are *not distinct* in this example.  
...
3. determinant = product of eigenvalues, $det(A) = \prod \lambda_i^2$.
```{r}
det(A)
prod(values)
```
