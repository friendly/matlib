...
```{r class3}
X <- as.matrix(class[,c(3,2,4,5)])
head(X)
```

## Orthogonalization by projections

The Gram-Schmidt process treats the variables in a give order, according to the columns in `X`.
...
- The Type I tests given by `anova()` should be the same^[In theory.  I'm not sure why some of the `Sum Sq` differ in this example.]
