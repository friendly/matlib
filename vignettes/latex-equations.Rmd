...
```{r eqn2,results='asis'}
Eqn("\\mathbf{I}_3 =\n", latexMatrix(diag(3), matrix="bmatrix"))
```
...
As the above examples illustrate, the matrix delimeters can be varied using the `matrix` argument.
...
$\mathbf{Y} = \mathbf{X} \mathbf{B} + \mathbf{E}$ with $q$ predictors,
$\mathbf{x}_1, \mathbf{x}_2, \dots, \mathbf{x}_q$ and 
$p$ responses,
...
with
three responses $y_1, y_2, y_3$ and three predictors $x_1, x_2, x_3$, the coefficients $\mathbf{B}$ are given by
...
the coefficients for the corresponding rows of $\mathbf{B}$ are zero using a 2-row $\mathbf{C}$ matrix that
...
Then, the hypothesis to be tested can be expressed as:
