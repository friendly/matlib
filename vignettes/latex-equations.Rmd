...
```{r eqn3,results='asis'}
latexMatrix(matrix(LETTERS[1:4], nrow=1)) |> Eqn()
```

```{r eqn4,results='asis'}
latexMatrix(matrix(letters[1:3], ncol=1)) |> Eqn()
```
...
label (using the `label` argument) and equations can be numbered (using `number=TRUE`).
...
Eqn(latexMatrix("u", "n", "k", lhs = ''),
    ...
    latexMatrix("v", "k", "p", transpose = TRUE), number = TRUE)
```
