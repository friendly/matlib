...
```{r eqn3,results='asis'}
latexMatrix(matrix(LETTERS[1:4], nrow=1)) |> Eqn(number = FALSE)
```

```{r eqn4,results='asis'}
latexMatrix(matrix(letters[1:3], ncol=1)) |> Eqn(number = FALSE)
```
...
using `Eqn()` and `latexMatrix()`. In Rmd markup, `Eqn()` can be given an equation label.
...
The matrix names in Equation `r ref("eq:svd")` are printed in a **boldface** math font, typically used for matrices
and vectors. Note that when using
LaTeX code in R expressions each backslash (`\`) must be doubled (`\\`) in R because `\` is the
escape character.

Note that the first equation can be referenced because it was labeled:  "As seen in Equation `r ref("eq:svd")` \ldots ".
...
The transpose, `t()`, and matrix product, `%*%` of `"latexMatrix"` objects are similarly straightforward:
...
Eqn(M %*% t(D), number=FALSE)
```

The determinant is computed recursively by finding the cofactors of the first row of the matrix. The method is applicable to a matrix of any order, although beyond an order-3 matrix, the resulting expression gets very long. A couple of examples:
```{r determinant}
A
determinant(A)
M
...
Eqn(determinant(M), number=FALSE)
```
...
Eqn(solve(M, simplify=TRUE), number=FALSE)
```
We can also supply values for the elements of the matrix to obtain a numeric inverse:
```{r numeric-inverse}
MASS::fractions(as.numeric(solve(M), locals=c(a=1, b=0, c=-2, d=-4, e=7, f=-1, g=0, h=4, i=6)))
MASS::fractions(det(as.numeric(M, locals=c(a=1, b=0, c=-2, d=-4, e=7, f=-1, g=0, h=4, i=6))))
```
