...
The `symbolicMatrix()` function can also generate symbolic equations from numeric or character matrices.
...
Then use `symbolicMatrix()` on `cbind(A,b)` and pipe the result to `Eqn()`:

```{r results='asis'}
symbolicMatrix(cbind(A,b)) |> Eqn(number=FALSE)
```

To demonstrate that this is an augmented matrix the command `\bigm|` can be included.

```{r results='asis'}
symbolicMatrix(cbind(A, "\\bigm|", b)) |> Eqn(number=FALSE)
```
