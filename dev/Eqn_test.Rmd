...
like Pythagoras' theorem, $a^2 + b^2 = c^2$ (`a^2 + b^2 = c^2`) once you learn
the notation. Writing equations with arrays, matrices and vectors is
somewhat more challenging. Many people rely on interactive \LaTeX\
editors like [Overleaf](https://www.overleaf.com/), [MathType](https://www.wiris.com/en/mathtype/), or online
versions that provide a menu-driven interface with fill-in templates for
matrices.

There are alrady some tools available in R for producing \LaTeX\ output for
tables (`xtable::xtable()`), R objects (`Hmisc::latex()`)
and statistical models (the [`equatiomatic`](https://cran.r-project.org/package=equatiomatic) package.)
...
are $x_{ij}, i = 1, 2, \cdots, n; j = 1, 2, \cdots,m$ in a \LaTeX\ `\begin{pmatrix} ... \end{pmatrix}`
...
sign. Note the locations of the `&` for alignment, specified as the left-hand side (`lhs`)
...
Create character matrix and vector:
```{r}
A <- matrix(paste0('a_', 1:9), 3, 3, byrow = TRUE) |> print()
b <- paste0("\\beta_", 1:3) |> print()
```

Using `matrix2latex()` we can show the matrix $[\mathbf{A} | \mathbf{b}]$. Note that you can pipe the result of `matrix2latex()` to `Eqn()`:
